...
<h2>平行計算 2023/08/20 更新</h2>

平行計算涉及同時執行多個進程，從而實現更快速和更高效的計算。
於 2023 年 8 月 20 日，我們進行了一個實驗，以比較平行計算與傳統的單線程方法的執行時間。

<h3>實驗設置</h3>

我們採用了遺傳算法來解決特定的優化問題。遺傳算法由一組候選解組成，每個解都表示為一個遺傳字符串。

這些候選解會隨著時代的推移而演化，通過選擇、交叉和突變等過程，最終旨在找到最佳解決方案。

在我們的實驗中，我們使用了一個擁有四個核心的 CPU，這使得我們能夠充分利用平行計算。

我們對比了在單線程中運行遺傳算法與在平行模式下運行時所需的執行時間，進行了 20 次迭代。

為了解決Epoch 參數導致每次Fitness 計算所需的LSTM 模型訓練時間不同的問題，我們進行了分別實驗10 次單線程和平行運算，然後取其平均值，最後得到每種運算方式所需的平均時間。

這個設計旨在準確比較單線程和平行計算的性能，同時考慮了 Epoch 參數的影響。

<h3>結果</h3>

我們實驗的結果是具有啟發性的。當我們在單個線程中執行遺傳算法時，它大約需要平均 1445.75 秒才能完成所有必要的時期。

然而，當我們使用平行計算時，執行時間減少。整個過程約在平均 1180.39 秒內完成。這代表著執行時間減少了約 18%。

但是，這與預期的理論優化幅度存在差距，理論上，如果有 n 個線程，執行時間可以減少到單線程執行時間的 1/n。

但在實際情況中，存在一些額外的開銷，例如線程之間的通信和同步，這可能會導致實際減少幅度小於理論值。

這個結果表明平行計算在減少執行時間方面確實有效，但在特定情況下，性能的提升可能會受到一些因素的限制。

進一步的優化和調整可能需要考慮這些因素，以更好地利用多核心計算資源。
